...
const {shuffle} = require("../service/shuffler");
...
router.get('/playlist', ensureAuthenticated, async function (req, res) {
    ...
    const playlist = await fetchAllPages(req.session.access_token, '/playlists/7at9lCk1NMBgJQI2v24fi3/tracks?fields=next,offset,items(track(uri))')
