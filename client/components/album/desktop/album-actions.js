...
import {addToPlaylist as addToPlaylistAction, markAlbumAsSeen as markAlbumAsSeenAction} from '../../../redux/actions/seen-albums'
    ...
    return (
        <div className={bem()}>
            ...
            <button className={`${bem('action')} ${bem('button')}`} onClick={() => {
                ...
                <img src='album-actions/reaper.png' alt=''/> {seenAlbums.includes(album.id) ? 'Mark as Unseen' : 'Mark as Seen'}
            </button>
        </div>
    )
}
